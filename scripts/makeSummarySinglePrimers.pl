...
while(<SAMPLESHEET>){
	my @words = split("\t", $_);
	my $sample = $words[0];
	my $file = "results/$sample/$sample.$primer.clusters.selected.fasta";
	if (-s $file){
		open(IN, "$file");
		while(<IN>) {
			chomp $_;
...
while(<SAMPLESHEET>){
	...
	$file = "results/$sample/$sample.$primer.clusters.selected.fasta";
			...
			if (/^>(.+)/){
				...
				if ($info[2] eq "good" and $freq1[1] > $cutoff) {
					...
					if ($info[2] =~ /1bpVariant/){
						@info1 = split(":", $info[2]);
						# print "$sample $id: $info[2] $info1[0] $info1[1] $foldchange \n";
						if ($info1[1] < $foldchange and $freq1[1] > $cutoff){
