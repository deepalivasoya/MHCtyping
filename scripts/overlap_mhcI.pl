...
open(MERGED_INFO_OUT, ">$work_dir/$sample.mhcI.overlapped.info.txt") or die "Cannot write $work_dir/$sample.mhcI.overlapped.info.txt\n";
...
if ($flag1 eq 1 and $flag2 eq 1){
		...
		foreach my $seq2 (sort {$primer2_order{$a} <=> $primer2_order{$b}} keys %primer2){
			...
			if ($overlapping1 eq $overlapping2){
				...
				foreach $id1 (@ids1){
					foreach $id2 (@ids2){
						if ($id1 eq $id2){
							$new_id = "$id1|$info1[1]-$info1[2]|$info2[1]-$info2[2]";
				...
				if ($id_check eq 1){
					...
					$count++;
					$overlapped = $seq1.(substr $seq2, -32);
					$new_id = "$sample-new$count|$info1[1]-$info1[2]|$info2[1]-$info2[2]";
					print MERGED_OUT ">$new_id\n$overlapped\n";
					print MERGED_INFO_OUT "$new_id\t$full_id1\t$full_id2\n";
	...
	foreach my $seq1 (sort {$primer1_order{$a} <=> $primer1_order{$b}} keys %primer1){
		...
		if (!exists $overlapping_primer1{$full_id1}){
			...
			chop $new_id;
			print MERGED_OUT ">$new_id|$info1[1]-$info1[2]|0-0\n$seq1\n";
			print MERGED_INFO_OUT "$new_id|$info1[1]-$info1[2]|0-0\t$full_id1\n";
	...
	foreach my $seq2 (sort {$primer2_order{$a} <=> $primer2_order{$b}} keys %primer2){
		...
		if (!exists $overlapping_primer2{$full_id2}){
			...
			chop $new_id;
			print MERGED_OUT ">$new_id|0-0|$info2[1]-$info2[2]\n$seq2\n";
			print MERGED_INFO_OUT "$new_id|0-0|$info2[1]-$info2[2]\t$full_id2\n";
